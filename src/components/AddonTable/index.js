  ...
  const sortedData = useMemo(() => {
    ...
    if (sortConfig !== null) {
      sortableItems.sort((a, b) => {
        ...
        // Handle price sorting specifically if needed (remove currency symbols)
        if (sortConfig.key === 'price') {
            // Simple check for "Free" vs numbers
          ...
          <tbody>
            {filteredData.map((addon, index) => (
              <tr key={index}>
                ...
                <td>
                  {addon.tags && addon.tags.map((tag, i) => (
                    <span 
                      key={i} 
                ...
                <td>
                  {addon.download_link && (
                    <a href={addon.download_link} target="_blank" rel="noopener noreferrer" className={styles.downloadLink}>
                      Link
                    </a>
                  )}
                </td>
            ...
            {filteredData.length === 0 && (
              <tr>
                <td colSpan="6" style={{textAlign: 'center'}}>No addons found matching your filter.</td>
              </tr>
            )}
          </tbody>
        </table>
